...
var DisplayVariablesConcept = function(name, options) {
    ...
    knockout.track(this, ['items', 'isOpen', 'allowMultiple', 'requireSomeActive']);
...
DisplayVariablesConcept.prototype.toggleActiveItem = function(concept) {
    ...
    if (isActive && this.requireSomeActive && this.items.every(child => child === concept ? child.isActive : !child.isActive )) {
        return;
    }
    // If we are activating an item, but only allow one selection, we need to deselect the others.
    if (!isActive && !this.allowMultiple || shouldClearActives(this, concept.id)) {
    ...
    if (defined(this.toggleActiveCallback && typeof this.toggleActiveCallback === 'function')) {
...
function shouldClearActives(parent, selectedId) {
    ...
    var activeChildIds = parent.activeItems.map(function(child) { return child.id; });
    var activeExclusiveIds = activeChildIds.filter(function(activeId) { return parent.exclusiveChildIds.indexOf(activeId) >= 0; });
