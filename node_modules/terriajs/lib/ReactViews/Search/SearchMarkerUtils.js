...
const SEARCH_MARKER_DATA_SOURCE_NAME = 'TerriaJS Search Marker Points';

/**
 * Adds a search marker to the map with the position supplied in the result, adding a data source to terria if one hasn't
 * already been added, and removing all previously added markers in that data source. This data source is stored in
 * viewState.searchState.mapPointerDataSource.
 */
export function addMarker(terria, viewState, result) {
    if (!viewState.searchState.mapPointerDataSource) {
        viewState.searchState.mapPointerDataSource = new CustomDataSource(SEARCH_MARKER_DATA_SOURCE_NAME);
    }
 ...
 */
export { SEARCH_MARKER_DATA_SOURCE_NAME };
