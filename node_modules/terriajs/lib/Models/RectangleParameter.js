...
RectangleParameter.prototype.formatValueAsString = function(value) {
    ...
    return Math.abs(swlat).toFixed(2) + '°' + (swlat < 0 ? 'S ' : 'N ') +
           Math.abs(swlon).toFixed(2) + '°' + (swlon < 0 ? 'W' : 'E') + ' to ' +
           Math.abs(nelat).toFixed(2) + '°' + (nelat < 0 ? 'S ' : 'N ') +
           Math.abs(nelon).toFixed(2) + '°' + (nelon < 0 ? 'W' : 'E');
};
...
RectangleParameter.formatValueAsString = RectangleParameter.prototype.formatValueAsString;
RectangleParameter.getGeoJsonFeature = RectangleParameter.prototype.getGeoJsonFeature;
