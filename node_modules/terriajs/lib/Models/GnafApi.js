...
var DATA61_GNAF_BULK_SEARCH_URL = 'http://gnaf.nationalmap.nicta.com.au/v2/bulkSearch';
...
GnafApi.prototype.geoCode = function(searchTerm, rectangle, maxResults) {
    ...
    return loadWithXhr({
        ...
        var hits = data.hits.map(convertLuceneHit.bind(undefined, defined(rectangle)));
        return hits;
    }.bind(this));
};
    ...
    return loadWithXhr({
        ...
        for (var i=0; i<data.length; i++) {
            ...
            if (defined(response.hits)) {
                hits.push(response.hits.map(convertLuceneHit.bind(undefined, defined(rectangle)))[0]);
            }
        }
        return hits;
    }.bind(this));
...
function convertLuceneHit(locational, item) {
    ...
    return {
        ...
        name: item.d61Address.slice(0, 3).filter(function(string) {
