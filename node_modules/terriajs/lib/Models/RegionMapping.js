...
var RegionMapping = function(catalogItem, tableStructure, tableStyle) {
    ...
    knockout.getObservable(this._catalogItem, 'useOwnClock').subscribe(function() {
        if (defined(this.clock)) {
            // If switching to own clock, first set it to match the timeline's clock.
            if (this._catalogItem.useOwnClock) {
                this.clock.currentTime = this._catalogItem.terria.clock.currentTime;
            } else {
                this._catalogItem.terria.clock.currentTime = this.clock.currentTime;
            }
            // Also, change the clock subscription to match the clock source.
            updateClockSubscription(this);
            this.showDataForTime(this.clock.currentTime);
        }
    }, this);
};
...
function calculateImageryLayerIntervals(timeColumn, currentTime, animateForwards) {
    ...
    var isBefore = animateForwards ? JulianDate.lessThan : JulianDate.greaterThan;
    var isAfter = animateForwards ? JulianDate.greaterThan : JulianDate.lessThan;
    ...
    timeColumn.timeIntervals.forEach(function(interval) {
        ...
        [interval.start, interval.stop].forEach(function(time) {
 ...
 * @param {JulainDate} currentTime The current time in the datasource.
 * @param  {Array} [regionIndices] Passed into setNewRegionImageryLayer. Saves recalculating it if available.
 */
function redisplayRegions(regionMapping, currentTime, regionIndices) {
    if (defined(regionMapping._regionDetails)) {
...
function updateClockSubscription(regionMapping) {
    ...
    // Only if there is a time column.
    // If the underlying catalog item has polling, updates occur through the activeItems getObservable instead.
    var needSubscription =
        regionMapping.hasActiveTimeColumn &&
        catalogItem.isEnabled &&
        catalogItem.isShown &&
        !catalogItem.useOwnClock;

    if (needSubscription && !defined(regionMapping._clockTickSubscription)) {
    ...
    } else if (!needSubscription && defined(regionMapping._clockTickSubscription)) {
        // Unsubscribe
